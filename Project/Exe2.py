...
if __name__ == '__main__':
    ...
    ##################
    # Monte Carlo :
    price_evolution = price_evolution_with_MC_method(_spot_price, _strike_price, _volatility, _maturity,
                                                     _risk_free_rate,
                                                     _nb_sequences)[:, -1]
    ...
    # Price
    call_price_MC_IS = np.mean(np.maximum(price_evolution_IS - _strike_price, 0) * radon_nikodym_der) * np.exp(
        -_risk_free_rate * _maturity)
    digital_call_price_MC_IS = np.mean(digital_call_payoff_IS * radon_nikodym_der) * np.exp(
        -_risk_free_rate * _maturity)

    print(
        'The price for this call with MC and importance of sampling far from the money is {}'.format(call_price_MC_IS))
    # Result : The price for this call with MC and importance of sampling far from the money is 1.0820880961709169e-259
    print('The price for this digital call with MC and importance of sampling far from the money is {}'.format(
        digital_call_price_MC_IS))
